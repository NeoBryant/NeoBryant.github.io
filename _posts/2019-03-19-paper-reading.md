...
量化一个了均值为$$s$$的伯努利分布与另一个均值为 $$\hat{s}_j$$ 伯努利分布之间的差异。总之，$$h$$是隐藏层中的神经元数，索引 $$j$$ 是对所有隐藏神经元进行求和。如果 $$s_{j}=s$$，则该惩罚具有 的特性，否则随着 $$s_{j}$$ 远离 $$s$$ 而增加。因此，最小化该项具有使 $$s_j$$ 接近 $$s$$ 的功能。 我们在这里使用的代价函数是     
