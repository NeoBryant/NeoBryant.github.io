...
​	我们不是第一个考虑 CNN传统神经元模型的替代方案。例如，Jarrett等人声称非线性$$f(x)=\mid tanh(x)\mid$$特别适用于它们的对比度或不失真类型，其次是 Caltech-101 数据集的局部平均池（local average pooling）。然而，在这个数据集上，主要关注的是防止过拟合，因此他们观察到的效果不同于我们在ReLUS时报告的加速适应训练集的能力。快速学习对大型数据集上训练的大型模型的性能有很大影响。
...
其中 $$i$$ 是迭代下标，$$v$$ 是动量变量，$$\epsilon$$ 是学习率，$$\langle\frac{\partial L}{\partial w}\mid_{w_{i}}\rangle_{D_i}$$是关于 $$w$$ 的目标导数的第 $$i$$ 批次 $$D_i$$的平均值，在 $$w_i$$处评估。
